...
envDir = 'assets/cubemap/'
var envmap = [
	envDir + 'GraceCathedral',
	envDir + 'Indoor',
	envDir + 'Skybox',
	envDir + 'CornellBox',
	envDir + 'Skybox2',
...
async function GAMES202Main() {
	...
	let envObjs = new Object();
	for (let i = 0; i < envmap.length; i++) {
		let demo = envmap[i].substring(envDir.length);
		envObjs[demo] = i;   
	}
	...
	function createGUI() {
		...
		panelModel.add(guiParams, 'envmapId', envObjs).name('Envmap Name');
		panelModel.open();
	}
