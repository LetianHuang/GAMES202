...
namespace ProjEnv
{
    ...
    /**
     * @param images vector 6 x (height * width * channel) float[] is a vector that flattened from a image tensor
     * @param width (1 of 6 images in cudemap)'s width
     * @param height (1 of 6 images in cubemap)'s height
     * @param channel (1 of 6 images in cubemap)'s channel
     * @return vector<Eigen::Array3f> SHNum x 3 dtype=float SHCoeffiecents
    */
        ...
        for (int i = 0; i < 6; i++)
        {
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    float omega = CalcArea(x, y, width, height);

                    for (int l = 0; l <= (int) SHOrder; l++) {
                        for (int m = -l; m <= l; m++) {
                            double sh = sh::EvalSH(l, m, dir.cast<double>().normalized());
                            SHCoeffiecents[sh::GetIndex(l, m)] += Le * sh * omega;
                        }
                    }
                }
            }
...
public:
    ...
    virtual void preprocess(const Scene *scene) override
    {
        ...
        for (int i = 0; i < (int) envCoeffs.size(); i++)
        {
        ...
        for (int i = 0; i < (int) mesh->getVertexCount(); i++)
        {
            const Point3f &v = mesh->getVertexPositions().col(i);
            const Normal3f &n = mesh->getVertexNormals().col(i);
            auto shFunc = [&](double phi, double theta) -> double {
                Eigen::Array3d d = sh::ToVector(phi, theta);
                const auto wi = Vector3f(d.x(), d.y(), d.z());// wi == wi.normalized()
            ...
            for (int j = 0; j < (int) shCoeff->size(); j++)
            {
        ...
        // Save in face format
        for (int f = 0; f < (int) mesh->getTriangleCount(); f++)
        {
